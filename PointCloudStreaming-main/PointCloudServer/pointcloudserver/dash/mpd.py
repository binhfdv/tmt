...
def append_new_frame(mpd_root, drc_filename, drc_size, fps):
    """
    Appends a new frame to the existing MPD dynamically.
    - Each frame is a new `Period` (for real-time streaming).
    - Uses `frame_id` to keep frames in sequence.
    """
     
    period_counter = len(mpd_root.findall("Period"))  # Get last used ID
    period = ET.SubElement(mpd_root, "Period", id=str(period_counter))
    ...
    adaptation_set = ET.SubElement(period, "AdaptationSet", id=str(period_counter))

    mime_type = infer_mime_type(drc_filename)
    if mime_type:
        adaptation_set.set("mimeType", mime_type)
    
    id        = str(uuid.uuid4())
    bandwidth = drc_size * int(fps)

    representation=ET.SubElement(adaptation_set, "Representation")
    representation.set("id", str(id))
    representation.set("size", str(drc_size))
    representation.set("bandwidth", str(bandwidth))

    representation_url=ET.SubElement(representation, "BaseURL")
     # Extract directory & filename from `drc_filename`
    frame_name = os.path.basename(drc_filename)  # e.g., "frame_0001.drc"
    parent_folder = os.path.dirname(drc_filename)  # e.g., "qp10"
    representation_url.text = f"{parent_folder}/{frame_name}"
